...
interface DiterimaItem {
  ...
  koordinator: { namaLengkap: string } | null;
}
...
interface DidistribusikanItem {
  ...
  keterangan: string | null;
  logistikItem: { id: string; namaBarang: string; satuan: string };
  pendukung: { namaLengkap: string; nik: string } | null;
}
...
export default function LogistikRelawanClient({
  relawanId,
  stokItems,
  initialDiterima,
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const res = await fetch("/api/relawan/logistik", {
      method: "POST",
    ...
    if (res.ok) {
      setShowModal(false);
      setForm({ logistikItemId: "", jumlah: "", tanggal: new Date().toISOString().slice(0, 10), pendukungId: "", keterangan: "" });
      router.refresh();
    } else {
  ...
  const tabs: { key: TabType; label: string; count: number }[] = [
    ...
    { key: "diterima", label: "Diterima dari Koordinator", count: diterima.length },
    { key: "distribusi", label: "Distribusi ke Pendukung", count: didistribusikan.length },
  ];
  ...
  return (
    <div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {stokItems.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">Belum ada logistik diterima dari koordinator</td></tr>
              ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b">
                ...
                <th className="px-4 py-3 text-left font-semibold text-gray-600 text-xs uppercase">Satuan</th>
                <th className="px-4 py-3 text-left font-semibold text-gray-600 text-xs uppercase">Dari Koordinator</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {diterima.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">Belum ada logistik diterima</td></tr>
              ) : (
                diterima.map((d, idx) => (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b">
                ...
                <th className="px-4 py-3 text-right font-semibold text-gray-600 text-xs uppercase">Jumlah</th>
                <th className="px-4 py-3 text-left font-semibold text-gray-600 text-xs uppercase">Satuan</th>
                <th className="px-4 py-3 text-left font-semibold text-gray-600 text-xs uppercase">Pendukung</th>
                <th className="px-4 py-3 text-left font-semibold text-gray-600 text-xs uppercase">NIK</th>
                <th className="px-4 py-3 text-left font-semibold text-gray-600 text-xs uppercase">Tanggal</th>
                <th className="px-4 py-3 text-left font-semibold text-gray-600 text-xs uppercase">Keterangan</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {didistribusikan.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400">Belum ada distribusi ke pendukung</td></tr>
              ) : (
      ...
      {/* Modal Distribusi ke Pendukung */}
      {showModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
            <form onSubmit={handleSubmit} className="p-6">
              <h2 className="text-xl font-bold mb-4">Distribusi ke Pendukung</h2>
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Pendukung Penerima *</label>
                  <select
                    value={form.pendukungId}
                    onChange={(e) => setForm({ ...form, pendukungId: e.target.value })}
                    required
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm"
                  >
